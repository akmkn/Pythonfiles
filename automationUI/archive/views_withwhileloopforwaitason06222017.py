...
def index(request):
    ...
    global RMSanity,Email,clientipaddress,datetimestamp,t0,t1,currenttime
       ...
       #ctypes.windll.user32.MessageBoxW(0, RMSanity, "Your Title", 1)
        if RMSanity == "RMSanity":
            ...
            if proceedtosubmitflag == True:
                ...
                #########################################################################################
                #totalscripttimeexpected = 2 * 60 * 60  #hard coded for time being
                totalscripttimeexpected = 60  # hard coded for time being
                start = time.time()
                currenttime = 0.0
                iscompletedflag = False
                while currenttime < totalscripttimeexpected:
                    end = time.time()
